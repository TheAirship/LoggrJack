...
A short FAQ for currently compatible log types are listed below. Commands, tips, and examples are listed in the following section, and caveats and reminders for using LoggrJack are listed after that.
...
Some logs include events that are benign and only clutter the log, such as FaultDomainRedirect in Microsoft 365 logs or HealthMailbox events in IIS. You can filter out these events for a cleaner log analysis with the -g parameter.

`$ python LoggrJack.py -l [path_to_log_file] -m [path_to_MaxMind_mmdb_file] -g`

You can also filter out all RFC 1918 (private), loopback, and APIPA / link local IPv4 and IPv6 addresses from analysis with the -P parameter.
...
* Does LoggrJack require an Internet connection to complete an analysis of my logs?
    ** In short, no. LoggrJack uses your Internet connection to determine your current county (as part of log analysis), to execute WHOIS lookups, and to pull down data from Have I Been Pwned. If you don't want to connect to the Internet, or a connection is unavailable...
...
Version and release information can be found in the Change Log file.
